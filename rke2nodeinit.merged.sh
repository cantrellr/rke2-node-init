...
Usage:
  ...
  sudo ./rke2nodeinit.sh [options] <image|push|server|add-server|agent|verify>   # pull & cluster-ca → image
  sudo ./rke2nodeinit.sh examples/pull.yaml


YAML kinds (apiVersion: rkeprep/v1):
  - kind: Image|Server|AddServer|Agent|Verify   # Pull & ClusterCA are now aliases of Image

Note: 'pull' and 'cluster-ca' actions are deprecated and now map to 'image'
---
...
# Action: CLUSTER-CA
action_cluster_ca() {
  log WARN "action_cluster_ca is deprecated; using action_image() to perform minimal image prep & caching."
  action_image
}
...
# Action: PULL
action_pull() {
  log WARN "action_pull is deprecated; using action_image() to perform minimal image prep & caching."
  action_image
}
