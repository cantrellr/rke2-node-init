...
#   - kind: Pull|pull, Push|push, Image|image, Server|server, AddServer|add-server|addServer, Agent|agent
#
# Exit codes:
...
extract_with_progress() {
  ...
  if command -v pv >/dev/null 2>&1; then
    ...
    tar $flags -C "$dest" -f "$tarfile" --checkpoint=.200 --checkpoint-action=echo="." >>"$LOG_FILE" 2>&1 &
    local pid=$!
...
Usage:
  ...
  sudo ./rke2nodeinit.sh [options] <pull|push|image|server|add-server|agent|verify>
