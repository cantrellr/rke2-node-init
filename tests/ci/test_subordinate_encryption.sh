...
${REPO_ROOT}/scripts/certs/generate-subordinate-ca.sh --input "${INPUT_PATH}" --out-dir outputs --encrypt-sub-key --sub-passfile "${SUB_PASSFILE_PATH}" --root-key "${ROOT_KEY_PATH}" --root-cert "${ROOT_CERT_PATH}" --root-passphrase "${ROOT_PASS}"
...
${REPO_ROOT}/scripts/certs/verify-chain.sh --root root/root-ca.crt --sub "${SUB_CERT}" --sub-key "${SUB_KEY}" --sub-passfile "${SUB_PASSFILE_PATH}" || { echo "verify-chain failed"; exit 1; }
