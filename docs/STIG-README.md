...
This document describes what `scripts/apply-stigs.sh` does, how to run it safely, and highlights additional tooling and manual tasks required to achieve full STIG compliance.
...
## Cluster‑level STIG hardening

After all Kubernetes workloads and Helm charts have been deployed, additional hardening is required at the namespace and network level.  The repository includes `scripts/apply-cluster-stigs.sh`, which can be executed on a management workstation with access to all cluster contexts.  This script applies post‑deployment STIG controls that cannot be enforced at image build time:

- **Namespace labels for Pod Security Standards** – Sets `pod-security.kubernetes.io/enforce=restricted` and `enforce-version=latest` on a set of application namespaces to ensure the Pod Security Admission controller blocks insecure workloads.
- **NetworkPolicy preparation** – Labels each namespace with `kubernetes.io/metadata.name=$namespace` to simplify label‑selector based policies.  Optionally, the script can deploy a default deny NetworkPolicy in each namespace.
- **Context discovery** – Iterates across all kubeconfig contexts by default, or only those specified with `--contexts`.  Namespaces can also be overridden via `--namespaces`.

Run this script only after verifying that application‑specific NetworkPolicies are in place to avoid service disruption.  See the script header for usage details.

## Manual actions

Not all STIG controls can be enforced automatically.  For a consolidated list of manual tasks—including operating system hardening, RKE2 parameter verification, and workload‑level controls—refer to `docs/STIG-MANUAL-ACTIONS.md`.  Use the template in `docs/STIG-CHECKLIST.md` to record evidence for each manual remediation.
