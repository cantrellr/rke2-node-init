...
2. **Single-node RKE2 clusters** using the same golden-image process plus `bin/rke2-single-node-profile.sh` to apply the single-node profile before the existing server engine runs.
...
single-node target host
  └─ bin/rke2-single-node-profile.sh server
...
The `server` action in `bin/rke2-single-node-profile.sh` exists to prevent operator sequencing mistakes. It applies the overlay first, then calls the existing server action. For single-node manifests, use the wrapper unless you are intentionally running the expanded two-step flow.
...
| Path | Purpose |
| --- | --- |
| `bin/rke2-single-node-profile.sh` | Applies/renders/verifies the single-node profile overlay and provides a `server` wrapper action. |
...
| `configs/cotpa-single-nodes/image-hyperv-v1.35.5+rke2r2-singlenode.yaml` | COTPA replacement-cluster golden image manifest. |
| `configs/cotpa-single-nodes/nodes/*.yaml` | COTPA replacement single-node server manifests. |
...
| `tests/test-single-node-profile.sh` | Lightweight syntax/render smoke test for the helper and COTPA manifests. |
...
The existing boot service path remains appropriate for standard multi-node manifests. For single-node clusters, use the wrapper command so the profile is applied before server provisioning:

```bash
sudo bash bin/rke2-single-node-profile.sh server -f <single-node-server.yaml> -y
```
...
Existing multi-node manifests do not need `spec.clusterMode` or `spec.singleNode`. The single-node helper refuses to run unless the manifest opts into single-node mode or `--force` is passed.
