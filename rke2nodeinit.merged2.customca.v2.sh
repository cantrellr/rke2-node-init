...
Actions:
  image        Prepare a *minimal* Ubuntu 24.04 image for air-gapped RKE2.
               ...
               - Optional: seed a custom cluster CA (interactive or via YAML)
...
YAML Spec (rkeprep/v1):
  ...
  spec:
    ...
    customCA:                         # optional; embed under kind: Image
      rootCert: /path/to/root.crt
      rootKey:  /path/to/root.key     # required unless using intermediateKey
      intermediateCert: /path/to/int.crt   # optional
      intermediateKey:  /path/to/int.key   # optional
      installToOsTrust: true          # optional; default true

YAML Spec (custom CA only):
  apiVersion: rkeprep/v1
  kind: custom_ca
  spec:
    rootCert: /path/to/root.crt
...
# --- Helpers for nested spec parsing and boolean normalization ---
yaml_spec_get_nested() {
  # Usage: yaml_spec_get_nested <file> <block> <key>
  # Example: yaml_spec_get_nested clusters/image.yaml customCA rootCert
  local file="$1" block="$2" key="$3"
  awk -v blk="$block" -v k="$key" '
    BEGIN { inSpec=0; inBlock=0; blockIndent=-1 }
    /^[[:space:]]*spec:[[:space:]]*$/ { inSpec=1; next }
    inSpec==1 {
      # If we hit a non-indented or top-level key, spec ends
      if ($0 ~ /^[^[:space:]]/) { exit }
      # Detect start of target block, capture its indent
      if (inBlock==0 && $0 ~ "^[[:space:]]+" blk ":[[:space:]]*$") {
        match($0, /^[[:space:]]+/); blockIndent=RLENGTH; inBlock=1; next
      }
      if (inBlock==1) {
        # If indent drops back to or before the block indent, block ended
        if ($0 ~ /^[[:space:]]*$/) { next }
        match($0, /^[[:space:]]+/); curIndent=RLENGTH
        if (curIndent<=blockIndent) { exit }
        # Look for target key
        if ($0 ~ "^[[:space:]]+" k "[[:space:]]*:") {
          line=$0
          sub(/^[[:space:]]+/, "", line)
...
bool_to_10() {
  # Normalize a possibly-human boolean to 1 or 0
  # echo 1 for: 1, true, yes, y (case-insensitive). Everything else -> 0
  local v="${1:-}"
  case "${v,,}" in
    1|true|yes|y) echo 1;;
    *) echo 0;;
...
maybe_handle_custom_ca_in_image() {
  # Decide whether to seed a custom cluster CA during 'image' prep.
  # Sources:
  #   - If -f points at kind: custom_ca -> read spec.*
  #   - If -f points at kind: Image with spec.customCA.* -> read inline block
  #   - Otherwise prompt the user interactively.
  local handled=0 kind=""

  if [[ -n "$CONFIG_FILE" && -f "$CONFIG_FILE" ]]; then
    kind="$(yaml_get_kind "$CONFIG_FILE" | tr '[:upper:]' '[:lower:]')"
    if [[ "$kind" == "custom_ca" ]]; then
      local rc rk ic ik trust
      rc="$(yaml_spec_get "$CONFIG_FILE" rootCert || true)"
      rk="$(yaml_spec_get "$CONFIG_FILE" rootKey || true)"
      ic="$(yaml_spec_get "$CONFIG_FILE" intermediateCert || true)"
      ik="$(yaml_spec_get "$CONFIG_FILE" intermediateKey || true)"
      trust="$(yaml_spec_get "$CONFIG_FILE" installToOsTrust || echo "true")"
      export CUSTOM_CA_ROOT_CRT="$rc"
      export CUSTOM_CA_ROOT_KEY="$rk"
      export CUSTOM_CA_INT_CRT="$ic"
      export CUSTOM_CA_INT_KEY="$ik"
      export CUSTOM_CA_INSTALL_TO_OS_TRUST="$(bool_to_10 "$trust")"
      handled=1
    elif [[ "$kind" == "image" ]]; then
      local rc rk ic ik trust
      rc="$(yaml_spec_get_nested "$CONFIG_FILE" customCA rootCert || true)"
      rk="$(yaml_spec_get_nested "$CONFIG_FILE" customCA rootKey || true)"
      ic="$(yaml_spec_get_nested "$CONFIG_FILE" customCA intermediateCert || true)"
      ik="$(yaml_spec_get_nested "$CONFIG_FILE" customCA intermediateKey || true)"
      trust="$(yaml_spec_get_nested "$CONFIG_FILE" customCA installToOsTrust || true)"
      if [[ -n "$rc" || -n "$rk" || -n "$ic" || -n "$ik" || -n "$trust" ]]; then
        export CUSTOM_CA_ROOT_CRT="$rc"
        export CUSTOM_CA_ROOT_KEY="$rk"
        export CUSTOM_CA_INT_CRT="$ic"
        export CUSTOM_CA_INT_KEY="$ik"
        [[ -n "$trust" ]] && export CUSTOM_CA_INSTALL_TO_OS_TRUST="$(bool_to_10 "$trust")"
        handled=1
      fi
    fi
  fi
  ...
  if (( handled == 0 )); then
    read -r -p "Seed a custom cluster CA now (chain RKE2 to your PKI)? [y/N]: " ans
    case "${ans,,}" in
      y|yes)
        local rc rk ic ik trust
        read -r -p "Path to ROOT CA certificate (PEM): " rc
        read -r -p "Path to ROOT (or INTERMEDIATE) private key (PEM): " rk
        read -r -p "Optional: Path to INTERMEDIATE certificate (PEM) [blank if none]: " ic
        [[ -n "$ic" ]] && read -r -p "Optional: Path to INTERMEDIATE private key (PEM): " ik
        read -r -p "Also install the root CA into OS trust store? [Y/n]: " trust
        trust="${trust:-Y}"
        export CUSTOM_CA_ROOT_CRT="$rc"
        export CUSTOM_CA_ROOT_KEY="$rk"
        [[ -n "$ic" ]] && export CUSTOM_CA_INT_CRT="$ic"
        [[ -n "$ik" ]] && export CUSTOM_CA_INT_KEY="$ik"
        if [[ "${trust,,}" == n* ]]; then
          export CUSTOM_CA_INSTALL_TO_OS_TRUST=0
        else
          export CUSTOM_CA_INSTALL_TO_OS_TRUST=1
        fi
        handled=1
        ;;
      *)
        log INFO "Skipping custom cluster CA seeding during image prep."
        return 0
        ;;
    esac
  fi
  ...
  if [[ -z "${CUSTOM_CA_ROOT_CRT:-}" ]]; then
    log WARN "custom_ca requested but rootCert missing; skipping."
    return 0
  fi
  if [[ ! -f "$CUSTOM_CA_ROOT_CRT" ]]; then
    log ERROR "Root cert not found: $CUSTOM_CA_ROOT_CRT ; skipping custom CA."
    return 0
  fi

  if [[ -n "${CUSTOM_CA_ROOT_KEY:-}" ]]; then
    if [[ ! -f "$CUSTOM_CA_ROOT_KEY" ]]; then
      log ERROR "Root key not found: $CUSTOM_CA_ROOT_KEY ; skipping custom CA."
      return 0
    fi
  elif [[ -n "${CUSTOM_CA_INT_CRT:-}" && -n "${CUSTOM_CA_INT_KEY:-}" ]]; then
    if [[ ! -f "$CUSTOM_CA_INT_CRT" || ! -f "$CUSTOM_CA_INT_KEY" ]]; then
      log ERROR "Intermediate cert/key not found; skipping custom CA."
      return 0
    fi
  else
    log WARN "No CA private key provided; cannot generate cluster CA. Skipping."
    return 0
  fi
...
NM
  ...
  # --- Optional: seed custom cluster CA during image prep ---
  maybe_handle_custom_ca_in_image || true
