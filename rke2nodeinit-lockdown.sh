...
# --- Role-specific pass-throughs ---
append_spec_config_server_extras() {
  local file="$1"; local cfg="/etc/rancher/rke2/config.yaml"
  ...
  for k in "${scalars[@]}"; do
    ...
    if [[ -n "$v" ]]; then
      if [[ "$v" =~ ^(true|false|[0-9]+)$ ]]; then echo "$k: $v" >> "$cfg"; else v="${v%\"}"; v="${v#\"}"; v="${v%\'}"; v="${v#\'}"; echo "$k: \"$v\"" >> "$cfg"; fi
    fi
  done
...
append_spec_config_agent_extras() {
  local file="$1"; local cfg="/etc/rancher/rke2/config.yaml"
  [[ -z "$file" || ! -f "$file" ]] && return 0
  _cfg_has_key() { grep -Eq "^[[:space:]]*$1[[:space:]]*:" "$cfg" 2>/dev/null; }
  ...
  # Agent-safe scalars
  local -a scalars=(
    "node-name" "with-node-id" "selinux" "lb-server-port" "protect-kernel-defaults"
    "container-runtime-endpoint" "default-runtime" "snapshotter" "private-registry"
    "node-ip" "node-external-ip" "resolv-conf" "write-kubeconfig-mode" "profile"
    "audit-policy-file" "pod-security-admission-config-file" "kubelet-path"
  )
  local k v
  for k in "${scalars[@]}"; do
    _cfg_has_key "$k" && continue
  ...
  # Agent-safe lists
  local -a lists=(
    "kubelet-arg" "kube-proxy-arg" "node-taint" "node-label" "tls-san"
  )
  for k in "${lists[@]}"; do
    _cfg_has_key "$k" && continue
    if yaml_spec_has_list "$file" "$k"; then
...
action_server() {
  ...
  append_spec_config_server_extras "$CONFIG_FILE"
