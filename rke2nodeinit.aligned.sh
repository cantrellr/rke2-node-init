...
action_server() {
  ...
  # --- helpers (local) ---
  _yaml_list_csv() { # best-effort: get list (or scalar) under spec.<key> as comma CSV
    local key="$1"
    local file="$CONFIG_FILE"
    [[ -z "$file" || ! -f "$file" ]] && return 0
    # try existing getter first
    local raw; raw="$(yaml_spec_get "$file" "$key" || true)"
    if [[ -n "$raw" ]]; then echo "$raw"; return 0; fi
    # naive block parse under 'spec:' then key:
    awk -v k="$key" '
      BEGIN{in_spec=0; in_list=0}
      /^spec:/ {in_spec=1; next}
      in_spec==1 && /^[^[:space:]]/ {in_spec=0}  # next top-level
      in_spec==1 {
        if ($0 ~ "^[[:space:]]*"k":[[:space:]]*$") { in_list=1; next }
        if (in_list==1) {
          if ($0 ~ "^[[:space:]]*-[[:space:]]") {
            sub(/^[[:space:]]*-[[:space:]]*/, "", $0); gsub(/^[[:space:]]+|[[:space:]]+$/,"",$0);
            if (length($0)>0) { items = (items==""?$0:items","$0) }
          } else if ($0 ~ "^[[:space:]]*[A-Za-z0-9_-]+:") {
            in_list=0
          }
        }
      }
      END{ if (length(items)>0) print items }
    ' "$file"
  }

  _emit_list_yaml() { # usage: _emit_list_yaml <key> <csv>
    local key="$1"; local csv="$2"
    [[ -z "$csv" ]] && return 0
    echo "$key:"
  ...
  # YAML keys under spec use EXACT RKE2 hyphen-case per docs (tls-san, token-file, node-ip, etc.).
  # Host networking still uses: ip, prefix, gateway, dns, searchDomains, hostname.
  # -------------------------
  local IP="" PREFIX="" HOSTNAME="" DNS="" SEARCH="" GW=""
  local TOKEN="" TOKEN_FILE="" AGENT_TOKEN="" AGENT_TOKEN_FILE="" SERVER_URL=""
  local TLS_SAN="" NODE_IP="" NODE_EXTERNAL_IP="" SERVICE_CIDR="" CLUSTER_CIDR="" CLUSTER_DOMAIN="" CLUSTER_DNS=""
  local NODE_LABELS="" NODE_TAINTS="" CNI="" DISABLE_CLOUD_CONTROLLER="" DISABLE_KUBE_PROXY="" ENABLE_SERVICELB="" INGRESS_CONTROLLER="" SYSTEM_DEFAULT_REGISTRY="" PRIVATE_REGISTRY=""
  local KUBELET_ARG="" KUBE_PROXY_ARG="" KAPIS_ARG="" KCM_ARG="" KSCHED_ARG="" ETCD_ARG="" WRITE_KUBECONFIG_MODE=""
  # Custom CA block (our extension)
  ...
  if [[ -n "$CONFIG_FILE" ]]; then
    ...
    # RKE2 exact keys (server/cluster/client/agent subsections all live at same level in config.yaml)
    SERVER_URL="$(yaml_spec_get "$CONFIG_FILE" server || true)"
    TOKEN="$(yaml_spec_get "$CONFIG_FILE" token || true)"
    TOKEN_FILE="$(yaml_spec_get "$CONFIG_FILE" token-file || true)"
    AGENT_TOKEN="$(yaml_spec_get "$CONFIG_FILE" agent-token || true)"
    AGENT_TOKEN_FILE="$(yaml_spec_get "$CONFIG_FILE" agent-token-file || true)"

    TLS_SAN="$(_yaml_list_csv "tls-san")"
    NODE_IP="$(yaml_spec_get "$CONFIG_FILE" node-ip || true)"
    NODE_EXTERNAL_IP="$(yaml_spec_get "$CONFIG_FILE" node-external-ip || true)"
    SERVICE_CIDR="$(yaml_spec_get "$CONFIG_FILE" service-cidr || true)"
    CLUSTER_CIDR="$(yaml_spec_get "$CONFIG_FILE" cluster-cidr || true)"
    CLUSTER_DOMAIN="$(yaml_spec_get "$CONFIG_FILE" cluster-domain || true)"
    CLUSTER_DNS="$(yaml_spec_get "$CONFIG_FILE" cluster-dns || true)"
    WRITE_KUBECONFIG_MODE="$(yaml_spec_get "$CONFIG_FILE" write-kubeconfig-mode || true)"

    NODE_LABELS="$(_yaml_list_csv "node-label")"
    NODE_TAINTS="$(_yaml_list_csv "node-taint")"
    CNI="$(yaml_spec_get "$CONFIG_FILE" cni || true)"
    DISABLE_CLOUD_CONTROLLER="$(yaml_spec_get "$CONFIG_FILE" disable-cloud-controller || true)"
    DISABLE_KUBE_PROXY="$(yaml_spec_get "$CONFIG_FILE" disable-kube-proxy || true)"
    ENABLE_SERVICELB="$(yaml_spec_get "$CONFIG_FILE" enable-servicelb || true)"
    INGRESS_CONTROLLER="$(yaml_spec_get "$CONFIG_FILE" ingress-controller || true)"
    SYSTEM_DEFAULT_REGISTRY="$(yaml_spec_get "$CONFIG_FILE" system-default-registry || true)"
    PRIVATE_REGISTRY="$(yaml_spec_get "$CONFIG_FILE" private-registry || true)"

    KUBELET_ARG="$(_yaml_list_csv "kubelet-arg")"
    KUBE_PROXY_ARG="$(_yaml_list_csv "kube-proxy-arg")"
    KAPIS_ARG="$(_yaml_list_csv "kube-apiserver-arg")"
    KCM_ARG="$(_yaml_list_csv "kube-controller-manager-arg")"
    KSCHED_ARG="$(_yaml_list_csv "kube-scheduler-arg")"
    ETCD_ARG="$(_yaml_list_csv "etcd-arg")"

    # Custom CA (extension; keep names stable under customCA:)
    CUSTOM_ROOT_CRT="$(yaml_spec_get "$CONFIG_FILE" customCA.rootCrt || true)"
    CUSTOM_ROOT_KEY="$(yaml_spec_get "$CONFIG_FILE" customCA.rootKey || true)"
    CUSTOM_INT_CRT="$(yaml_spec_get "$CONFIG_FILE" customCA.intermediateCrt || true)"
    CUSTOM_INT_KEY="$(yaml_spec_get "$CONFIG_FILE" customCA.intermediateKey || true)"
    INSTALL_TRUST="$(yaml_spec_get "$CONFIG_FILE" customCA.installToOSTrust || echo "true")"
  fi

  # CLI overrides (same as before; hyphen-case mapped to variables)
  local OPTIND=1 opt
  while getopts ":t:T:a:A:s:l:n:e:S:C:D:d:M:L:Q:K:W:Y:E:r:p:" opt; do
    case ${opt} in
      t) TOKEN="$OPTARG";;
      T) TOKEN_FILE="$OPTARG";;
      a) AGENT_TOKEN="$OPTARG";;
      A) AGENT_TOKEN_FILE="$OPTARG";;
      s) SERVER_URL="$OPTARG";;
      l) TLS_SAN="$OPTARG";;     # CSV
      n) NODE_IP="$OPTARG";;
      e) NODE_EXTERNAL_IP="$OPTARG";;
      S) SERVICE_CIDR="$OPTARG";;
      C) CLUSTER_CIDR="$OPTARG";;
      D) CLUSTER_DOMAIN="$OPTARG";;
      d) CLUSTER_DNS="$OPTARG";;
      M) WRITE_KUBECONFIG_MODE="$OPTARG";;
      L) NODE_LABELS="$OPTARG";;  # CSV
      Q) NODE_TAINTS="$OPTARG";;  # CSV
      K) KUBELET_ARG="$OPTARG";;  # CSV
      W) KUBE_PROXY_ARG="$OPTARG";; # CSV
      Y) CNI="$OPTARG";;
      E) ENABLE_SERVICELB="$OPTARG";;
      r) SYSTEM_DEFAULT_REGISTRY="$OPTARG";;
      p) PRIVATE_REGISTRY="$OPTARG";;
    esac
  done
  shift $((OPTIND-1))

  # Prompts for host basics if missing
  ...
  # Registry handling
  if [[ -n "$PRIVATE_REGISTRY" || -n "$SYSTEM_DEFAULT_REGISTRY" ]]; then
    setup_image_resolution_strategy
  else
  ...
  # -------------------------
  # Write /etc/rancher/rke2/config.yaml from EXACT keys
  # -------------------------
  mkdir -p /etc/rancher/rke2
...

  {
    ...
    [[ -n "$AGENT_TOKEN_FILE" ]] && echo "agent-token-file: \"$AGENT_TOKEN_FILE\""
    [[ -z "$AGENT_TOKEN_FILE" && -n "$AGENT_TOKEN" ]] && echo "agent-token: \"$AGENT_TOKEN\""

    [[ -n "$WRITE_KUBECONFIG_MODE" ]] && echo "write-kubeconfig-mode: \"$WRITE_KUBECONFIG_MODE\""
    [[ -n "$CNI" ]] && echo "cni: \"$CNI\""
    [[ -n "$DISABLE_CLOUD_CONTROLLER" ]] && echo "disable-cloud-controller: $DISABLE_CLOUD_CONTROLLER"
    [[ -n "$DISABLE_KUBE_PROXY" ]] && echo "disable-kube-proxy: $DISABLE_KUBE_PROXY"
    [[ -n "$ENABLE_SERVICELB" ]] && echo "enable-servicelb: $ENABLE_SERVICELB"
    [[ -n "$INGRESS_CONTROLLER" ]] && echo "ingress-controller: \"$INGRESS_CONTROLLER\""
    [[ -n "$SYSTEM_DEFAULT_REGISTRY" ]] && echo "system-default-registry: \"$SYSTEM_DEFAULT_REGISTRY\""
    [[ -n "$PRIVATE_REGISTRY" ]] && echo "private-registry: \"$PRIVATE_REGISTRY\""
    ...
    [[ -n "$CLUSTER_DNS" ]] && echo "cluster-dns: \"$CLUSTER_DNS\""

    [[ -n "$TLS_SAN" ]] && _emit_list_yaml "tls-san" "$TLS_SAN"
    [[ -n "$NODE_LABELS" ]] && _emit_list_yaml "node-label" "$NODE_LABELS"
    [[ -n "$NODE_TAINTS" ]] && _emit_list_yaml "node-taint" "$NODE_TAINTS"
    [[ -n "$KUBELET_ARG" ]] && _emit_list_yaml "kubelet-arg" "$KUBELET_ARG"
    [[ -n "$KUBE_PROXY_ARG" ]] && _emit_list_yaml "kube-proxy-arg" "$KUBE_PROXY_ARG"
    [[ -n "$KAPIS_ARG" ]] && _emit_list_yaml "kube-apiserver-arg" "$KAPIS_ARG"
    [[ -n "$KCM_ARG" ]] && _emit_list_yaml "kube-controller-manager-arg" "$KCM_ARG"
    [[ -n "$KSCHED_ARG" ]] && _emit_list_yaml "kube-scheduler-arg" "$KSCHED_ARG"
    [[ -n "$ETCD_ARG" ]] && _emit_list_yaml "etcd-arg" "$ETCD_ARG"
  } >> "$cfg"
  chmod 600 "$cfg"
  log INFO "Server config written to $cfg (keys aligned with RKE2 docs)"
...
action_agent() {
  ...
  # -------------------------
  local IP="" PREFIX="" HOSTNAME="" DNS="" SEARCH="" GW=""
  local URL="" TOKEN="" TOKEN_FILE="" NODE_IP="" NODE_EXTERNAL_IP="" TLS_SAN=""
  local NODE_LABELS="" NODE_TAINTS="" KUBELET_ARG="" KUBE_PROXY_ARG=""
  local AGENT_CA_CERT="" SYSTEM_DEFAULT_REGISTRY="" PRIVATE_REGISTRY=""
  ...
  if [[ -n "$CONFIG_FILE" ]]; then
    ...
    URL="$(yaml_spec_get "$CONFIG_FILE" server || true)"
    TOKEN="$(yaml_spec_get "$CONFIG_FILE" token || true)"
    TOKEN_FILE="$(yaml_spec_get "$CONFIG_FILE" token-file || true)"
    NODE_IP="$(yaml_spec_get "$CONFIG_FILE" node-ip || true)"
    NODE_EXTERNAL_IP="$(yaml_spec_get "$CONFIG_FILE" node-external-ip || true)"
    ...
    # CA to trust locally
    AGENT_CA_CERT="$(yaml_spec_get "$CONFIG_FILE" caCert || true)"
    [[ -z "$AGENT_CA_CERT" ]] && AGENT_CA_CERT="$(yaml_spec_get "$CONFIG_FILE" caCrt || true)"
  fi

  # CLI overrides for Agent (hyphen-case semantics via short flags)
  # -s URL  -t TOKEN  -T TOKEN_FILE  -n NODE_IP  -e NODE_EXTERNAL_IP  -l TLS_SANS  -C AGENT_CA_CERT
  # -L NODE_LABELS -Q NODE_TAINTS -K KUBELET_ARG -W KUBE_PROXY_ARG -r SYSTEM_DEFAULT_REGISTRY -p PRIVATE_REGISTRY
  local OPTIND=1 opt
  while getopts ":s:t:T:n:e:l:C:L:Q:K:W:r:p:" opt; do
    case ${opt} in
      s) URL="$OPTARG";;
      t) TOKEN="$OPTARG";;
      T) TOKEN_FILE="$OPTARG";;
      ...
      L) NODE_LABELS="$OPTARG";;
      Q) NODE_TAINTS="$OPTARG";;
      K) KUBELET_ARG="$OPTARG";;
      W) KUBE_PROXY_ARG="$OPTARG";;
      r) SYSTEM_DEFAULT_REGISTRY="$OPTARG";;
      p) PRIVATE_REGISTRY="$OPTARG";;
    esac
  done
  ...
  if [[ -z "$TOKEN" && -z "$TOKEN_FILE" ]]; then read -rp "Cluster join token (or leave blank if using token-file): " TOKEN || true; fi
  ...
  # -------------------------
  # Write /etc/rancher/rke2/config.yaml for agent join (aligned keys)
  # -------------------------
  mkdir -p /etc/rancher/rke2
  local cfg="/etc/rancher/rke2/config.yaml"
  if [[ -f "$cfg" ]]; then cp -f "$cfg" "${cfg}.bak.$(date +%s)" || true; fi
  ...
  chmod 600 "$cfg"
  log INFO "Agent join config written to $cfg (keys aligned with RKE2 docs)"
