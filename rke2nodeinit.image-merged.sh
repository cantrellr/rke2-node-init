...
#
# Actions:
#   1) #   2) push   - Tag and push preloaded images into a private registry (nerdctl only)
...
YAML kinds (apiVersion: rkeprep/v1):
  - kind: Push|Image|Server|AddServer|Agent|Verify
...
action_image() {
  ...
  # --- Cache nerdctl FULL for offline re-provisioning -----------------------
  ...
  run_rke2_installer "$SRC" "server"
    # Export artifacts into RUN_OUT_DIR (if available)
  if [[ -n "${RUN_OUT_DIR:-}" ]]; then
    ...
    if [[ -f /etc/rancher/rke2/rke2.yaml ]]; then
      cp -f /etc/rancher/rke2/rke2.yaml "$RUN_OUT_DIR/${SPEC_NAME}-kubeconfig.yaml"
      log INFO "Exported kubeconfig to $RUN_OUT_DIR/${SPEC_NAME}-kubeconfig.yaml"
    else
      log WARN "Kubeconfig not present yet (/etc/rancher/rke2/rke2.yaml). You can export it later to $RUN_OUT_DIR/${SPEC_NAME}-kubeconfig.yaml."
    fi
    # If custom CA provided, snapshot it
    if [[ -n "${CUSTOM_ROOT_CRT:-}" && -f "${CUSTOM_ROOT_CRT}" ]]; then cp -f "${CUSTOM_ROOT_CRT}" "$RUN_OUT_DIR/${SPEC_NAME}-root-ca.crt"; fi
  fi
