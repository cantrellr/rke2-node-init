...
Actions:
  image        Prepare a *minimal* Ubuntu 24.04 image for air-gapped RKE2.
               - Optionally seed a *Custom Cluster CA* via `-f` YAML (kind: custom_ca)
                 or interactively; if omitted, custom_ca is skipped.
...
YAML Spec (rkeprep/v1):
  ...
  # Custom Cluster CA definition (consumed by the `image` action)
  apiVersion: rkeprep/v1
  kind: custom_ca
  spec:
    rootCert: /path/to/kuberegistry-ca.crt        # required
    rootKey:  /path/to/kuberegistry-ca-key.pem    # required
    intermediateCert: /path/to/intermediate.crt   # optional
    intermediateKey:  /path/to/intermediate-key.pem # optional
...
Options:
  -f FILE     YAML config (may be an action kind like Image|Server|... or `custom_ca` for seeding CA)
...
# Handle optional Custom Cluster CA during 'image'
# Accepts either:
#   - YAML file with kind: custom_ca and spec fields:
#       rootCert: /path/to/root-ca.crt
#       rootKey:  /path/to/root-ca-key.pem
#       intermediateCert: /path/to/intermediate.crt   # optional
#       intermediateKey:  /path/to/intermediate-key.pem # optional
#       installToOsTrust: true|false                  # optional, default true
#   - Or interactive prompts if no YAML provided / not of kind custom_ca.
maybe_handle_custom_ca_in_image() {
  local yaml="${1:-}"
  local kind=""
  if [[ -n "$yaml" && -f "$yaml" ]]; then
    kind="$(yaml_get_kind "$yaml" 2>/dev/null || true)"
    kind="${kind,,}"
  fi

  # helper to resolve relative paths vs YAML location
  _resolve_path() {
    local base="$1" p="$2"
    ...
    local d="."
    if [[ -n "$base" ]]; then d="$(cd "$(dirname "$base")" && pwd -P)"; fi
    (cd "$d" && printf "%s" "$(pwd -P)/$p")
  }

  local ROOT_CRT="" ROOT_KEY="" INT_CRT="" INT_KEY="" OS_TRUST=""

  if [[ "$kind" == "custom_ca" ]]; then
    ROOT_CRT="$(_resolve_path "$yaml" "$(yaml_spec_get "$yaml" rootCert || true)")"
    ROOT_KEY="$(_resolve_path "$yaml" "$(yaml_spec_get "$yaml" rootKey  || true)")"
    INT_CRT="$(_resolve_path "$yaml" "$(yaml_spec_get "$yaml" intermediateCert || true)")"
    INT_KEY="$(_resolve_path "$yaml" "$(yaml_spec_get "$yaml" intermediateKey  || true)")"
    OS_TRUST="$(yaml_spec_get "$yaml" installToOsTrust || true)"
    if [[ -z "$ROOT_CRT" || -z "$ROOT_KEY" ]]; then
      log ERROR "custom_ca YAML missing rootCert/rootKey; skipping custom CA."
      return 0
    fi
  else
    # offer interactive prompt
    echo
    read -rp "Pre-seed a custom cluster CA now? [y/N]: " _ans || true
    case "${_ans,,}" in
      y|yes)
        read -rp "Path to ROOT CA certificate (.crt): " ROOT_CRT
        read -rp "Path to ROOT CA private key (.pem): " ROOT_KEY
        read -rp "Path to INTERMEDIATE certificate (.crt) [optional]: " INT_CRT || true
        read -rp "Path to INTERMEDIATE private key (.pem) [optional]: " INT_KEY || true
        read -rp "Install root CA into OS trust store? [Y/n]: " _yn || true
        case "${_yn,,}" in n|no) OS_TRUST="false";; *) OS_TRUST="true";; esac
        ;;
      *) return 0;;
    esac
  fi

  # Export for setup_custom_cluster_ca()
  export CUSTOM_CA_ROOT_CRT="$ROOT_CRT"
  export CUSTOM_CA_ROOT_KEY="$ROOT_KEY"
  [[ -n "$INT_CRT" ]] && export CUSTOM_CA_INT_CRT="$INT_CRT" || unset CUSTOM_CA_INT_CRT
  [[ -n "$INT_KEY" ]] && export CUSTOM_CA_INT_KEY="$INT_KEY" || unset CUSTOM_CA_INT_KEY
  if [[ -n "$OS_TRUST" ]]; then
    if [[ "${OS_TRUST,,}" =~ ^(1|y|yes|true)$ ]]; then
  ...
  if [[ ! -f "$CUSTOM_CA_ROOT_CRT" ]]; then
    log ERROR "Root CA certificate not found: $CUSTOM_CA_ROOT_CRT"
    return 0
  fi
  if [[ ! -f "$CUSTOM_CA_ROOT_KEY" ]]; then
    log ERROR "Root CA key not found: $CUSTOM_CA_ROOT_KEY"
    return 0
  fi

  log INFO "Setting up custom cluster CA (root=$(basename "$CUSTOM_CA_ROOT_CRT"))."
  setup_custom_cluster_ca || log WARN "Custom cluster CA setup failed; continuing without it."
}
action_image() {
...
NM
  ...
  # ------------------ Custom Cluster CA (optional) ------------------
  # Accept custom CA via YAML (-f with kind: custom_ca) or interactively prompt.
  maybe_handle_custom_ca_in_image "${CONFIG_FILE:-}" || true
